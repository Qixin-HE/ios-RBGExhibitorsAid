    ...
    @IBAction func saveLocation(_ sender: Any) {
        guard let lat = Double(latitudeTextField.text!),
            let long = Double(longitudeTextField.text!)else{
                let alertController = UIAlertController(title: "Co-ordinates invalid",message: "Lat & Long must be numbers", preferredStyle: .alert)
                alertController.addAction(UIAlertAction(title: "Dismiss", style:
                    .default, handler: nil))
                present(alertController, animated: true, completion: nil)
                return
        ...
        let newLocation = LocationAnnotation(title: titleTextField.text!,subtitle:descriptionTextField.text!, lat: lat, long: long)
